...
#read bamboo page list
def get_facebook_hompage_url():
    adrres = []
    with open('bamboo_list.csv','r') as file:  # , encoding='utf8'
        w = csv.reader(file)
        for line in w:
            adrres.append(line[1])
    adrres = list(map( lambda x: re.sub('https://www.facebook.com/','',x), adrres))
    adrres = list(map(lambda x : re.sub('/','',x),adrres))
    return adrres

adrres =  get_facebook_hompage_url()
...
def processFacebookPageFeedStatus(status):
    ...
    com = fetch_comments(status)

    return (status_message, status_published, num_likes, com)

#######CSV######
def write_csv(wan_data, num, page_id):
    with open('data %s.csv' % (page_id), 'wt', encoding = 'cp949', newline='') as file:  #, encoding='utf8'
        w = csv.writer(file)
        w.writerow(['content_num', 'content_dates', 'content_hits', 'content', 'comments'])
        for i in range(num):
            w.writerow([i, wan_data[i][1], wan_data[i][2],wan_data[i][0], wan_data[i][3]])
            # w.writerow([wan_data[6][0],wan_data[6][1],wan_data[6][2],wan_data[6][3]])


def fetch_feed():
    for page_id in adrres :
        ...
        while True:
            try:
                ...
                try:
                    ...
                    break
        write_csv(wan_data, num, page_id)
